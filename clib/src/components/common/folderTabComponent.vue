      ...
      <div class="menu_tab_line_detail">
        <div v-for="(item, index) in compm" v-bind:key="index">
          <div
            ...
            @click="ChangeComponent(item.menuId, index)"
          >
            <span id="tab_nm">
              {{ item.menuNm }}
            </span>
            <span
              v-if="this.comp === item.menuId"
              class="tab_x_on"
              @click.prevent.stop="DeleteComponent(index)"
              >x</span
            >
            <span
              v-else
...
export default {
  ...
  props: {
    folderPath: {       //AddComponent의 defineAsyncComponent에서 컴포넌트를 호출할 위치를 지정하기 위한 변수
      type: String,
    },
    compArray: {        //컴포넌트배열
      type: Array,
  ...
  methods: {
    ChangeComponent: function (componentName, index) {  //클릭된 컴포넌트로 이동
          ...
          if (this.compm.length < 10) {
            ...
            this.component = markRaw(                     //추가될 컴포넌트를 import를 통해 불러온다. 고정된 컴포넌트의 경우 이방식을 통해 사전에 불러와야한다.
              defineAsyncComponent( () => 
                import(`@/${this.folderPath}/${this.compm[this.cur_num].menuId}.vue`) 
              )
            );
